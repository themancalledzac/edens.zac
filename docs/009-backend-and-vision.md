...
- **Email/SES production posture:** the [email/SES production posture](superpowers/specs/2026-07-06-email-ses-production.md) doc lays out the ordered console checklist (domain identity + Easy DKIM in us-west-2, MAIL FROM/SPF, DMARC `p=none`, configuration set + SNS bounce handling, sandbox-exit request text included), code changes C1–C10 (S/M/L-sized), and decisions 1–6. **The spec's as-is section is now out of date** — it was written 2026-07-06, and the backend built invite email on 2026-07-26 (`352dafb`). What is actually true on backend `main`: `EmailService` has two public send methods, `sendGalleryPasswordEmail` and `sendInviteEmail`; `AdminUserController.sendInviteEmailAfterCommit` calls the second one from three sites, registering a `TransactionSynchronization` so the send fires in `afterCommit` and a rolled-back invite cannot mail a link to a token that was never persisted (with no active transaction, as in standalone MockMvc tests, it sends immediately). The remaining blocker is operational, not code: `EMAIL_ENABLED` defaults to `false` in both `docker-compose.yml` and `.env.example`, so `sendInviteEmail` short-circuits to a log line and returns `email-disabled`. Until that flips, the invite response still carries `inviteUrl` and the admin copies the link by hand — `InviteLinkResult.tsx` on this side is the copy-to-clipboard UI, and it stays useful either way, because the emailed link and the copied link are byte-identical by construction.
- **Passkey login — diagnosed broken e2e (spike):** the [passkey login diagnosis](spikes/2026-07-06-passkey-login-diagnosis.md) verified a 4-layer root cause — the attempt-cookie (`Path=/api/auth/webauthn`) never survives the BFF proxy so login/finish always 401s (**passkeys have never worked end-to-end anywhere**); no enrollment surface exists for password-only accounts; prod serves `rpId=localhost` (compose lacks `WEBAUTHN_*` passthrough); and the shared login limiter compounds it. Fixes shipped on both sides: [#210](https://github.com/themancalledzac/edens.zac/pull/210) merged `0211-passkey-login-fixes` (FE: `/user` AccountCard enrollment, flow-keyed login errors, invite-failure surfacing), and the backend leg is on its `main` — `WebAuthnController.ATTEMPT_COOKIE_PATH` is `"/"` and `docker-compose.yml` plumbs `WEBAUTHN_RP_ID` / `WEBAUTHN_RP_NAME` / `WEBAUTHN_ALLOWED_ORIGINS`. Not yet verified end-to-end against a deployed environment. Follow-up: a credentials list/remove backend endpoint for enrollment-state UI.
...
| [Email/SES production posture](superpowers/specs/2026-07-06-email-ses-production.md)     | Go-live checklist + code changes           | 📘 ⚠️ as-is section stale — invite email shipped 2026-07-26                          |
| [Passkey login diagnosis](spikes/2026-07-06-passkey-login-diagnosis.md)                  | Spike — root cause + fixes                 | ✅ fixes merged ([#210](https://github.com/themancalledzac/edens.zac/pull/210) + BE) |
