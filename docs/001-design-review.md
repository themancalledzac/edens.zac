...
> Collapse the fractured UI surface (~30–38 button classes, 5 modal shells, 2 filter bars, 3 badge defs, 8 copy-pasted page scaffolds, 3 color families) into ~10 token-driven primitives in `app/components/ui/`, on one painted dark-safe surface — plus the IA/UX + a11y gaps that ride along. · 8 sections · ✅ **fully shipped to `main`** (PRs [#152](https://github.com/themancalledzac/edens.zac/pull/152) → [#160](https://github.com/themancalledzac/edens.zac/pull/160), 2026-06-01 → 06-03). Only two intentionally-deferred carve-outs remain (`@custom-media` bridge + gap-rule/`rgb()` sweeps) — now owned by [006 · Code Health](006-code-health.md).

This chapter takes a genuinely well-built portfolio — one render pipeline, one excellent BoxTree layout engine, a real spacing/radius/type/z-index token foundation — and fixes the _concentrated foundation gaps_ sitting on top of it: no shared UI primitives, three competing color systems, navigation built on the wrong element, and a page that never paints its own background (so OS dark mode renders text invisibly). The single highest-leverage move is to stop re-implementing buttons/modals/chips/badges per feature and promote ~10 canonical primitives on one color taxonomy and one painted surface. The work decomposes into a master plan plus seven sub-plans that parallelize well: Phase 0 (foundation) lands first, then the primitives, then nav/shell, IA/UX, and finally the color-token collapse + a11y polish.

## ✅ Work done — all phases shipped (`0145` → `0159` · 2026-06-01 → 06-03)

**Phase 0 — Foundation** (`0145-design-phase-0-foundation`, [PR #152](https://github.com/themancalledzac/edens.zac/pull/152)):

- Bound `body { color: var(--color-fg) }` (the background already shipped in the 0144 fullscreen work) + declared the ~12-token compatibility **alias layer** in `globals.css` (`--text-primary`, `--border-color`, `--color-accent`, `--color-warning`, the `--color-overlay-light-80` ramp gap, …) so silent hex fallbacks stop winning.
- SiteHeader title/menu use `var(--color-fg)` instead of hardcoded `black` (no text-shadow scrim — the title sits on the white page bg, not over imagery).
- Removed dead `/all-blogs` link (MenuDropdown + adminTiles), the duplicate "Create" menu item, and the never-wired `onBack` prop (About → no-prop, ContactForm → onSubmit-only). _(Note: the review's claim that the nav cleanup already shipped was stale — only FsDebug had shipped.)_

**Button family** (`0146-design-button-primitives`, branched off `0145`):

- New `app/components/ui/` primitives: **`<Button>`** (variant/size/loading/leftIcon), **`<IconButton>`** (aria-label required), **`<CloseButton>`** — token-driven, Server-Component-friendly, **12 unit tests** under `tests/components/ui/`.
- `secondary` variant = grey-filled (`--color-secondary`); cancel-style actions use `ghost`.
- Migrated (clean light-surface matches; each test/lint/type-verified, one commit per file):
  - ClientGalleryGate gate CTA → `primary`
  - CollectionListSelector "Add New Child" → `secondary` (`sm`)
  - ClientGalleryDownload "Download All" + picker CTAs → `primary` (`leftIcon`)
- Convention adopted: **omit default props** — no `variant="primary"` / `type="button"` on `<Button>` (only non-default values are passed).

**Scoping decided** (deferred _with reasons_, not silently skipped):

- Dark-modal/overlay buttons (ImageMetadataModal, TextBlockCreateModal, ImageDownloadOverlay, fullscreen close) → migrate **with the Modal primitive (Task 1.2)** + Phase 4 scrim tokens — the 4 variants are light-surface only.
- Outline / text-link / light-on-dark buttons (AllImagesClient retry, gallery Cancel, ErrorBoundary retry, Comments delete, ManageClient stepper) → need an `outline` variant or Phase 4 tokens.
- `error.tsx` / `not-found.tsx` buttons → **Phase 2 `<StatusPage>`**; MenuDropdown close → deliberate hamburger-mirror, left alone.
- **Still fits, not yet done:** ManageClient (~7/8 buttons), Comments (reply/loadMore → `ghost`).

**Modal + Filters + Dropdown** (`0148-design-modal-migration`):

- **`<Modal>`** primitive shipped (portal, scrim, Esc, focus trap + return, `useBodyScrollLock`, `role=dialog`) + 7 unit tests; migrated **ClientGalleryGate** (overlay), **TextBlockCreateModal** (overlay), **ImageMetadataModal** (sheet — kills the `position:absolute + 100vh + top:scrollPosition` footgun), **FullScreenModal** (fullscreen). **MenuDropdown was migrated then reverted (`174f5fb`)** — it's a popover, not a blocking modal.
- **`<FilterToolbar>` / `<FilterChip>`** unified the 2 filter bars + 6 chip impls into one `FilterState`; **`<Dropdown<T>>`** promoted from `UnifiedMetadataSelector` with portable SCSS.
- **Post-migration fix (`ca8e8ad`):** the Modal migration introduced a fullscreen scroll-restore regression (double `useBodyScrollLock` + a per-instance-offset bug → jump-to-top) and a double backdrop. Fixed by hoisting the scroll offset to module level, making `<Modal>` the sole locker, and a single `0.9` scrim. Details: [modal plan → Post-migration fixes](superpowers/plans/001-modal-primitive.md).

**Nav, Shell & remaining primitives** (`0149-nav-and-shell`, [PR #156](https://github.com/themancalledzac/edens.zac/pull/156)):

- Shipped the rest of the primitive set into `app/components/ui/`: **`Tile`/`NavLink`** (always a real `<a>` — fixes the `div onClick` nav), **`PageShell`/`CollectionHeader`** (collapses the 8 copy-pasted page scaffolds, owns the painted surface), **`StatusPage`** (404/error on the safe surface), **`Badge`** (curated public labels, no raw `CollectionType`), the **`Field`** form set, and a generic **`MetadataList<T>`** (collapses the 3 near-identical metadata lists).
- **Admin-route gating** (`0150`, [PR #157](https://github.com/themancalledzac/edens.zac/pull/157)): gated `/all-collections` + `/all-images` behind admin auth (the real security fix the review flagged for `/metadata`-class routes).

**IA & UX** (`0151-ia-and-ux`, [PR #158](https://github.com/themancalledzac/edens.zac/pull/158)):

- Public taxonomy front door, deep-linkable fullscreen (`?image=<id>` + history) with a position counter and Back-closes-modal, wired the previously-orphaned `parse/serializeFilterToParams` URL helpers into the gallery clients, footer + breadcrumb, entity NAME on taxonomy pages.

**Color-token collapse & a11y** (`0152-token-collapse-a11y`, [PR #159](https://github.com/themancalledzac/edens.zac/pull/159) — Phase 4, runs last):

- Collapsed the 3 color families + ~15-token shadow namespace + the `--color-overlay-light-*` ramp + inline `rgb()` literals into one semantic taxonomy + `--scrim-{dark,light}-*`; added `--duration-*`/`--ease-standard`; one `--focus-ring`, banned bare `outline:none`, real per-page `h1`, `prefers-reduced-motion` guard. 12 dead tokens deleted post-ship. **Carve-outs deferred:** `@custom-media` breakpoint bridge (Next 16 postcss conflict) + gap-rule/`rgb()`-slash sweeps → folded into [006](006-code-health.md).

**ImageMetadataModal decomposition** (`0159`, [PR #160](https://github.com/themancalledzac/edens.zac/pull/160) — the chapter's final consumer migration): _Since renamed to **`MetadataModal`** (dir `app/components/Metadata/`, hooks `useMetadataState`/`useMetadataSubmit`) by the "drop the Image prefix" cleanup ([#170](https://github.com/themancalledzac/edens.zac/pull/170)) — historical names below are preserved as the record._

- Decomposed the 1099-LoC god-component → **203 LoC** orchestrator + `useImageMetadataState`/`useImageMetadataSubmit` hooks + 5 section subcomponents, all on the `ui/` primitives; deleted the 4 raw `*Button` classes PR #158 had carved out. +75 net-new component tests (the surface had ~0 before). Follow-on fixes after merge: dark-surface Save/Cancel visibility (`739fd6d`), fullscreen overlay alignment via `--fs-matte` (`2d30a7f`), and the camera optimistic-create race ([#162](https://github.com/themancalledzac/edens.zac/pull/162), now owned by [006](superpowers/plans/006-camera-optimistic-create-race.md)).

---

## Remaining work — only the deferred carve-outs (now owned by [006](006-code-health.md))

Everything in the original plan shipped. What's left was **intentionally deferred** out of Phase 4, not skipped:

- **`@custom-media` breakpoint bridge** — bridge the existing `--breakpoint-*` tokens so the ~100 hardcoded `768px` queries become token-driven. Deferred: postcss/Next 16 pipeline conflict. → [006 · Code Health](006-code-health.md).
- **Gap-rule + `rgb()`-slash syntax sweeps** — the two mechanical CSS sweeps (Phase 4 Tasks 6d/6e). → [006 · Code Health](006-code-health.md).

## Sections

| Section                                                                                           | Role | Status                                                     |
| ------------------------------------------------------------------------------------------------- | ---- | ---------------------------------------------------------- |
| [Master plan](superpowers/plans/001-design-system-unification.md)                                 | plan | ✅ all phases shipped                                      |
| [Filter toolbar & chip](superpowers/plans/001-filter-toolbar-and-chip.md)                         | plan | ✅ `0147`/`0148` (#154/#155)                               |
| [Modal primitive](superpowers/plans/001-modal-primitive.md)                                       | plan | ✅ `0148` (#155) (MenuDropdown reverted; fix in `ca8e8ad`) |
| [Dropdown primitive](superpowers/plans/001-dropdown-primitive.md)                                 | plan | ✅ `0147` (#154)                                           |
| [Nav & shell](superpowers/plans/001-nav-and-shell.md)                                             | plan | ✅ `0149` (#156) + admin gating `0150` (#157)              |
| [IA & UX](superpowers/plans/001-ia-and-ux.md)                                                     | plan | ✅ `0151` (#158)                                           |
| [Token collapse & a11y](superpowers/plans/001-token-collapse-and-a11y.md)                         | plan | ✅ `0152` (#159) — carve-outs → 006                        |
| [Image-metadata-modal decomposition](superpowers/plans/001-image-metadata-modal-decomposition.md) | plan | ✅ `0159` (#160)                                           |

## Open product decisions — resolved in practice

- ~~Light vs intentional-dark theme~~ → **light theme kept** (canvas now painted from tokens).
- ~~Persistent nav vs hamburger-only~~ → **persistent footer added** without removing the hamburger.

---

_↑ [Back to the book](000-summary.md). Source review preserved below._
...
> **Date:** 2026‑05‑31
> **Method:** Live visual tour (dev server :3001, desktop + mobile, every page type) + an 11‑agent read‑only code review (6 page‑clusters × 5 cross‑cutting dimensions: UX flows, component reuse, design tokens, SCSS, a11y). Findings below are cross‑corroborated between what was _seen_ and what was _read in the source_.
> **Through‑line:** how to collapse _many_ divergent design standards into _one_ unified, simple, reusable set of **Main Design Elements** — and simplify the logic underneath while we do it.

---

## 0. The one‑paragraph verdict

This is a genuinely well‑built photography portfolio with a **strong, already‑unified core** (one render pipeline, one excellent layout engine, a real token foundation) sitting under a **fractured surface layer** (no shared UI primitives, three competing color systems, navigation built on the wrong element). None of the problems are deep or architectural — they are _concentrated foundation gaps_. The single highest‑leverage move is to stop re‑implementing buttons/modals/chips/badges per feature and promote ~10 canonical **Main Design Elements**, sitting on **one** color‑token taxonomy and **one** painted page surface. Do that and most of the failures below disappear at once, the code shrinks, and every future change becomes a one‑place edit.
...
- **First‑time visitor → discover work.** Lands on the editorial home (strong first impression). But the _only_ way deeper is clicking image tiles that aren't links, and the _only_ nav is one hamburger icon. There is no "Browse all work," no taxonomy directory, no footer. Discoverability rests entirely on one icon.
- **Drill into a collection → view fullscreen.** The leaf gallery is the site's best moment (immersive, full‑bleed). But on first paint you see the header then a **blank black void** until lazy‑gated images stream in. Opening an image into fullscreen is excellent _in‑modal_ (swipe, arrows, Esc, metadata panel) — but it's **React‑state‑only**: not shareable, and **Back exits the whole collection** instead of closing the modal (the biggest mobile journey gap).
...
### Best (keep, and build _on_ these)
...
| `TextBlockCreateModal.tsx`                           | A _second_ independent modal shell + third close‑button style.                                                                                                                                                                 |
...
### Duplicate (one concept, many implementations) → _the consolidation catalog_
...
| **Filter bars + chips**               | `CollectionFilterBar` (borderless, dropdowns, slider) vs `LocationFilterBar` (bordered pills, film/digital colors); **6 chip variants** total; `cycleDateSort` exists twice with _different_ cycle orders | one `<FilterToolbar>` + `<FilterChip active count tone state>` |
...
1. **Full‑bleed BoxTree galleries** — edge‑to‑edge, gapless, varied row compositions. Immersive, intentional, distinctly _not_ a generic grid. This is the brand; everything else should serve it.
...
3. **One render pipeline for everything** — home, leaf, parent, and client galleries all flow through the same code; parent children are converted to image models so there's no special‑case parent UI. _The unification you want is already true at the structural level — don't fragment it._
...
This is the centerpiece. The answer to _"what buttons/sliders/dropdowns should be Main Design Elements?"_ is a set of **~10 canonical primitives** living in one place (`app/components/ui/` + `app/styles/design-elements/`), all driven by **one** token taxonomy.
...
3. **Bind the canvas** (`html,body` background/color) — this _alone_ fixes the 404, header, admin, and gate‑card invisibility together. Add the `@custom-media` bridge so the existing `--breakpoint-*` tokens become usable.
...
- **A public taxonomy directory** — turn the existing `/metadata` _data_ into a public "Explore" index (tags / people / locations as `NavLink`s), and/or add index routes at `/tag`, `/location`, `/people`. The data and `[slug]` pages already exist; only the front door is missing.
...
- **Lean _into_ an intentional dark theme.** The site is _accidentally_ dark today (a bug) — but full‑bleed photography looks fantastic on an intentional dark canvas. Rather than fight dark mode, consider **committing to a token‑driven dark theme** (or a clean light one) — either way, make `color-scheme` _honest_. This turns the #1 failure into a deliberate design decision.
...
- **Replace the buried, doubled filter UI** with **one persistent/sticky `FilterToolbar`** that's discoverable on first paint and works identically on collection _and_ taxonomy pages.
...
_Appendix: this report is the synthesis of an 11‑agent code review (≈1M tokens) cross‑checked against a live desktop+mobile visual tour. Per‑agent raw findings (with exhaustive `file:line` evidence for every item above) are available on request._
