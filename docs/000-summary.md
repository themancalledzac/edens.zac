...
| [spikes/2026-summer-refactor.md](spikes/2026-summer-refactor.md) | The live cleanup board — one row per MR, grouped A–H. Its sibling [spikes/2026-features.md](spikes/2026-features.md) is the feature board; both are tracked with their directories. |
...
One perf epic (~9 ordered tasks). **SSR BoxTree partial ship ([#161](https://github.com/themancalledzac/edens.zac/pull/161))** eliminated the blank-on-load CLS void. Remaining: hero-byte cuts, narrow priority, blur placeholder, `will-change` scoping, render micro-opts — largely shipped on the feature board 2026-08-31 (PF1/PF3/PF8/PF10); the home-page static render is PF13. The GIF poster is done: `thumbnailUrl` lands as `poster=` on all three video sites.
**Sections:** [LCP critical review](superpowers/specs/002-lcp-critical-review.md) 📘 · [LCP & Lighthouse](superpowers/plans/002-lcp-and-lighthouse.md) · [re-render perf](superpowers/plans/002-performance-rerender.md) · [cache & revalidation](superpowers/plans/002-cache-and-revalidation.md) 🟡 _(superseded by feature board PF13)_
...
Refactor + tests + observability + deps. **Shipped & archived** (→ [previous-work.md](previous-work.md)): React 19 upgrade, DRY consolidation, thin-component extraction, entity-edit DRY, inline-JSX config, dependency upgrade, camera optimistic-create race. **What's left:** function decomposition, property-based + complex-hook tests, and the inherited 001 CSS sweeps. Error tracking is decided and shipped — CloudWatch, feature board PF6 (#391); alerting and dashboards remain.
...
2. **Turn email on** — the code is built; `EMAIL_ENABLED` defaults false pending SES domain verification and sandbox exit. → [009](009-backend-and-vision.md).
3. **Client-gallery BCrypt** — parked behind the gallery-password design pass (feature board EM4, decision #3); do not start it before that is answered. → [003](003-client-gallery-security.md).
4. **Standing queue**: mobile-admin Phase 3 · `staging` collection ([008](008-collection-admin.md) — seed migration + non-upload auto-parenting) · CloudFlare Phase 2 ([007](007-security-hardening.md)) · 006 tail (Sentry/CloudWatch, decomposition, property tests) · the admin-hub panel-width vs page-height adjudication ([005](005-layout.md)) · perf ⛔ backend-blocked · SaveHeart 44px tap target.
5. **Deferred by design**: A2 dynamic Home · Track D automation (auto-related, CLIP auto-tag — BE ML design doc exists, 0% built) · ABAC vision · liked-images.
