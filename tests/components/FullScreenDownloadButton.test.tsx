 ...
 *  - Picking a format navigates to the download URL (via `navigateTo`). Navigation — not
...
describe('FullScreenDownloadButton', () => {
  ...
  it('navigates to the web download URL when "Web" is picked', () => {
    ...
    expect(navigateTo).toHaveBeenLastCalledWith(downloadImageUrl(42, 'web'));
  });
  ...
  it('navigates to the original download URL when "Full Size" is picked', () => {
    ...
    expect(navigateTo).toHaveBeenLastCalledWith(downloadImageUrl(9001, 'original'));
  });
