...
describe('buildAtomic — input order is preserved', () => {
...
describe('buildAtomic — AR fitness', () => {
  it('produces a row AR within reasonable bounds of the target', () => {
    // For a typical mixed row, buildAtomic should produce a tree whose AR is within
  ...
  it('lands close to target 1.0 on a high-density mixed-orient row', () => {
    ...
    // buildAtomic enumerates direction assignments and picks the candidate closest to
  ...
  it('keeps the /a-mariners-game row 0 (2V+2H) at AR >= 1.0, rejecting the sub-1.0 nested-quad', () => {
    ...
    // must never be taller than wide, so the floor rejects it and buildAtomic picks the
...
describe('buildAtomic — equitable sizing', () => {
