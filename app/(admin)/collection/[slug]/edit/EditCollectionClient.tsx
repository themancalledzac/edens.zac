      ...
      <ol className={styles.blocksList}>
        {blocks.map((b) => (
          <li key={b.id} className={styles.blockItem}>
            <code className={styles.blockTypeCode}>{String(b.type)}</code>
            <span>#{b.orderIndex}</span>
            {b.type === "TEXT" && 'content' in b && typeof b.content === "string" && (
              <span className={styles.blockContent}>{String(b.content).slice(0, 60)}...</span>
            )}
          </li>
        ))}
      </ol>
 ...
 *
 * Client-side component that handles data fetching, loading states, error handling,
 * and provides editing interface for content collections.
 *
 * @param slug - Collection slug from route parameters
 * @returns Client component with collection editing interface
 */
export default function EditCollectionClient({ slug }: { slug: string }) {
